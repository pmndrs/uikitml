...
export const createInspectorContainer = (children: Object3D, onElementClick: (uid: string) => void) => {
  ...
  const highlightContainer = new Container({
    ...
    zIndex: 1000,
    borderColor: 'red',
    borderWidth: computed(() => (inspectorContainer.size.value?.[0] ?? 0) * 0.005),
  })
  const selectedContainer = new Container({
    ...
    zIndexOffset: 19,
    borderColor: 'yellow',
    borderWidth: computed(() => (inspectorContainer.size.value?.[0] ?? 0) * 0.0035),
  })
  ...
  inspectorContainer.addEventListener('pointerover', (e) => {
    ...
    highlightContainer.setProperties({
      visibility: 'visible',
      transformTranslateX: computed(() => transformation.value.translation.x / pixelSize - 0.5 * width.value),
      transformTranslateY: computed(() => -transformation.value.translation.y / pixelSize - 0.5 * height.value),
      transformTranslateZ: computed(() => transformation.value.translation.z / pixelSize),
