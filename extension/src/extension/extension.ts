...
function bringDocumentToFront(
  ...
  const targetEditor = openEditors.find((editor) => editor.document.uri.toString() === documentUri.toString())
  if (targetEditor) {
    // If the document is already open, reveal the range and focus on it
    const range = new vscode.Range(rangeInfo.start, rangeInfo.end ?? rangeInfo.start)
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('uikit-preview.showPreview', () => {
      ...
      if (!document || path.extname(document.fileName).toLowerCase() !== '.uikitml') {
        return
      }
      if (currentPanel) {
        ...
        if (currentPanel.viewColumn === vscode.window.activeTextEditor?.viewColumn) {
...
function createWebviewPanel(context: vscode.ExtensionContext, document: vscode.TextDocument): vscode.WebviewPanel {
  ...
  const panel = vscode.window.createWebviewPanel('uikitPreview', `${basename} (UIKit)`, vscode.ViewColumn.Two, {
    enableScripts: true,
    ...
    (message) => {
      ...
      if (command === 'request-content') {
        ...
        if (message.rangeInfo) {
          if (currentDocument) {
            const rangeInfo = {
              start: new vscode.Position(message.rangeInfo.start.line, message.rangeInfo.start.column),
              end: new vscode.Position(message.rangeInfo.end.line, message.rangeInfo.end.column),
            }
        ...
        console.log('Applying property changes:', JSON.stringify({ changedProperties, rangeInfo }, null, 2))
        ...
        if (currentDocument && rangeInfo) {
          ...
          if (editor) {
            ...
            const modifiedElement = applyInlineStyles(elementText, changedProperties)
            ...
            if (modifiedElement && modifiedElement !== elementText) {
              ...
              vscode.workspace.applyEdit(edit).then(() => {
                ...
                if (currentDocument) {
                  vscode.window.showTextDocument(currentDocument, editor.viewColumn).then(() => {
          ...
          .then((uri) => {
            if (uri) {
              ...
              if (extension === '.js' || extension === '.jsx') {
                const fileString = `export const uiConfig = /** @type {const} */ (${JSON.stringify(json, null, 2)});`
                fs.writeFileSync(uri.fsPath, fileString)
                vscode.window.showInformationMessage('UIKit config (JS) generated successfully!')
              } else if (extension === '.ts' || extension === '.tsx') {
                const fileString = `export const uiConfig = ${JSON.stringify(json, null, 2)} as const;`

                fs.writeFileSync(uri.fsPath, fileString)
                vscode.window.showInformationMessage('UIKit config (TS) generated successfully!')
              }
            }
          })
      }
...
async function updateWebviewContent(panel: vscode.WebviewPanel, document: vscode.TextDocument) {
  const basename = path.basename(document.fileName)
...
export function applyInlineStyles(elementText: string, changedProperties: Record<string, any>): string | null {
...
function startLanguageServer(context: vscode.ExtensionContext) {
  ...
  client = new LanguageClient('uikitMLLanguageServer', 'UIKitML Language Server', serverOptions, clientOptions)
